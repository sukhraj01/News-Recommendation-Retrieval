...
Scores via a sparse matrix-vector product against the precomputed weight
matrix — see index.py's module docstring for why: the rank_bm25-native
per-token Python loop is too slow at real corpus/user-count scale.
"""
import numpy as np
...
def retrieve_top_k(index: BM25Index, query_tokens: list[str], k: int) -> list[str]:
    ...
    scores = (query_vec @ index.weights_t).toarray().ravel()
