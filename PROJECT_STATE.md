...
| EB-NeRD treatment (job 2694988, + freshness) | 🟡 Running on gnode012, ~27 min |
| Failed jobs, all on one broken node | 2694505 (CUDA OOM, fixed), then 2694963 / 2694964 / 2694986 — all instant failures on **gnode033**, whose driver is mismatched (`nvidia-smi` exits 18). Resubmissions use `--exclude=gnode033` and `--dependency=afterany` chaining |
