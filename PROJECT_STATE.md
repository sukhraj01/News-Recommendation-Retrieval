...
| 2 | MIND leaderboard screenshot | 🟡 **Blocked on the one remaining human step — everything else done.** `submissions/mind_large_test_nrms_official_treatment/prediction.zip` is generated and fully validated (2,370,727 lines, exact match to the known MINDlarge_test scale; 0 malformed lines, 0 duplicate impression ids, every rank list a genuine permutation, sha256-verified Ada↔local; `prediction.txt` at the zip root per Codabench's required format). Job history: 2696298 failed fast on a bad node (`gnode066`, CUDA init error despite a working `nvidia-smi` — a real, node-specific fault); the retry (2699322) hit a real deployment gap (Ada's `$HOME/a2/repo` was missing `src/submission`/`src/evaluation`/`src/datasets`/`src/utils` — only `src/retrieval` had ever been pushed there; fixed by pushing the full `src/` tree); the third attempt completed clean. **The actual Codabench upload requires the engineer's own account/login — true for all 6 prior submissions in this project (README.md, this file's Leaderboard Submission row) and remains true here.** Not attempted; not something Claude Code can do regardless of readiness. Once uploaded: capture the leaderboard screenshot into this same `submissions/` directory and record the real score here + in ADR-015, per every prior submission's convention |
