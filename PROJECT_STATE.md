...
| **Q5 extended eval** | 🟡 **Slicing done** (ADR-007 addendum, 2026-09-12): head/tail + warm/cold, per-slice paired CIs, 11 tests. **Key finding: EB-NeRD's freshness gain reverses on head articles** — head (n=9,541) Δ−0.0217 [−0.0258, −0.0178] vs tail (n=235,106) Δ+0.0076, so the +0.0064 headline is a tail gain diluted by a CI-clear head loss. MIND control: head 0.7411 / tail 0.6474, warm 0.6932 / cold 0.6209 (the reproduced baseline does **not** close the cold gap). Remaining: apply to the MIND A/B once its treatment lands |
...
| **Q6 design note** | 🟡 **Drafted** (`docs/design_note_a2.tex`, 6 pages, compiles clean via `tectonic`, verified via `pypdf`). Covers Q1, Q2 (the literal harness + its interim result, not just citations), Q3 A/B framing for both datasets, Q5 slicing, Q9 ablation, Q4 serving/scale, and the 10× scaling argument |
...
| 2 | MIND leaderboard screenshot | 🟡 **Blocked on the one remaining human step — everything else done and re-confirmed present on disk.** `submissions/mind_large_test_nrms_official_treatment/prediction.zip` generated and fully validated (2,370,727 lines, 0 malformed, 0 duplicates, every rank list a genuine permutation, sha256-verified Ada↔local). Job history: 2696298 failed fast on a bad node (`gnode066`); the retry (2699322) hit a real deployment gap (Ada's `$HOME/a2/repo` was missing most of `src/`); the third attempt completed clean, 111.6 min. **The Codabench upload requires the engineer's own account/login** — true for all 6 prior submissions, remains true here; not attempted. Once uploaded: capture the screenshot into `submissions/mind_large_test_nrms_official_treatment/` and record the real score here + in ADR-015 |
...
| 4 | AI usage log current through today | ✅ `knowledge/ai-usage-log/2026-09-11_a2-kickoff-corpus-stats-gap-analysis.md` — Prompts 1–10 verbatim plus the autonomous-execution phase (2026-09-15–17) that shipped the treatment and closed Q2 |
| 5 | Working tree fully committed and pushed | ✅ Re-verified 2026-09-17: `git status` clean (only the pre-existing, unrelated untracked `.claude/`), `git fetch` shows local `a2-q3-official-baseline` matches `origin/a2-q3-official-baseline` exactly, HEAD at `c1e5570` |
