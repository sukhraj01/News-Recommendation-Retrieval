...
| Q2 true-retrieval-ceiling eval | 🟡 **Harness built and verified** (`scripts/a2_q2_retrieve_rerank_eval.py`, 2026-09-14) — reuses A1's retriever + `OfficialNRMSScorer`, no new statistics. **NRMS-only, decided 2026-09-12** — K's features are impression-conditional and undefined for retrieved-but-unshown candidates (ADR-015). Correctness verified: harness's own freshly-measured hit rate (3.70% BM25, MINDsmall-dev) lands next to ADR-006's independent 2.62% citation for the same corpus. Interim result only, with a reduced-scale local checkpoint (Ada unreachable this session): after−before AUC −0.0464 [−0.1154, +0.0246], not significant — undertrained-checkpoint evidence, not a verdict on re-ranking. Full-scale re-run queued (`results/a2_q2/README.md`) for once `mind_treatment_v2` (job 2695890) is reachable |
