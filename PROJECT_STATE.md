...
**Last Updated:** August 12, 2026 (EB-NeRD Codabench submission — Part 0 resolved via Kaggle, Part 1 converter built and validated against ebnerd_small)

**Current Phase:** MIND Codabench submission (Q5) Parts 1-4 complete on the local side (see prior session notes below). EB-NeRD Codabench submission (competition 2469): Part 0 is now fully resolved — the engineer ran `notebooks/ebnerd_part0_kaggle_investigation.py` on Kaggle and relayed real results, confirming `predictions.txt`'s `impression_id [rank_1,...,rank_N]` format (same shape as MIND's), `articles_large_only.zip`'s 100% coverage of the test set's in-view articles, and the 200,000 `is_beyond_accuracy` rows' fixed-250-pool structure (needing real rankings too, no special-cased handling). Part 1 is now also complete: `src/submission/ebnerd_format.py` built as a direct port of `mind_format.py`, unit-tested against the `ebnerd_demo_sample.zip` fixture, and validated end-to-end against `evaluation/official/evaluate.py` (confirmed generic/dataset-agnostic in Part 0, not MIND-specific) on `ebnerd_small`'s validation split for both BM25 and embeddings — AUC/nDCG match `ranking_metrics.py`'s own numbers almost exactly; the small MRR gap is the same verified official-vs-project MRR-definition difference already documented for MIND, this time additionally traced to a genuine EB-NeRD data quirk (duplicate entries within a single impression's `article_ids_clicked`).

**Current Objective:** Part 2 — the real Kaggle test-set run (`ebnerd_testset.zip`, reusing `articles_large_only.zip` and the same `wget`-into-`/kaggle/working` approach Part 0 used), now that Part 1's converter is validated and worth trusting at real scale. Separately, MIND's Q6 design note and the manual Codabench upload/screenshot (`submissions/mind_large_test_embed/prediction.zip`) remain outstanding from the prior session.
...
| 3 | Leaderboard screenshots (both Codabench competitions) | 🟡 In progress (MIND) / 🟡 In progress (EB-NeRD, Part 1 done, Part 2 next) | MIND: Parts 1-4's local half complete — `submissions/mind_large_test_embed/prediction.zip` ready to upload. **The upload itself needs the engineer's own Codabench account/login, which Claude Code cannot do regardless of local readiness.** EB-NeRD (competition 2469): Part 0 fully resolved via Kaggle this session (real format, real test-set schema, 100% article coverage from `articles_large_only.zip` alone); Part 1 (converter + `ebnerd_small` validation) complete this session. Part 2 (the real `ebnerd_testset.zip` run on Kaggle) and Part 3 (submit + screenshot, engineer's own account) remain. |
| 4 | AI usage log (prompts + AI-vs-human marking) | 🟢 Ongoing | `knowledge/ai-usage-log/` exists; one file per session (`YYYY-MM-DD_<topic>.md`), written live per CLAUDE.md's "Prompt & Session Logging" section, not reconstructed after the fact (this session's log: `2026-08-12_ebnerd-codabench-part1-converter.md`) |
...
- [ ] Part 2 (real `ebnerd_testset.zip` run on Kaggle, reusing `articles_large_only.zip` and the same `wget` approach) and Part 3 (submit + screenshot): next up, Part 1's validation now makes Part 2 worth doing
...
## August 12, 2026 (latest) — EB-NeRD Codabench Submission, Part 0 Resolved + Part 1 Converter Built & Validated
