...
\small \url{https://github.com/sukhraj01/News-Recommendation-Retrieval}}
\date{\small \today}
...
from A1's own generator, $K\sim100$--$200$, re-ranked, with before/after metrics
reported. This is built as a literal, runnable harness
(\texttt{scripts/a2\_q2\_retrieve\_rerank\_eval.py}), not left as a citation:
A1's retriever pulls the top-$K$ candidates from the \emph{whole corpus} per
sampled impression using the user's real history as the query, and the trained
\texttt{OfficialNRMS} re-ranks that retrieved list. The retrieval ceiling is
measured fresh on the same sample rather than re-cited from an earlier
benchmark run, via the same bootstrap machinery as every other metric in this
project. A1's independently measured recall@200 (2.62--2.78\% MIND, 1.21--2.77\%
EB-NeRD) means at least $\sim$97\% of clicked articles never enter a top-200
list from either retriever --- no re-ranker can recover a click the candidate
generator never surfaced, and the harness's own freshly-measured hit rates
confirm this order of magnitude directly (3.70\%, CI 3.05--4.37\%, BM25 on a
3,000-impression MINDsmall-dev sample).

\textbf{The full-scale run: a large, CI-clear win for re-ranking.} Using the real
treatment checkpoint (10 epochs, MINDlarge, dev AUC 0.6868 --- matching \S2.2's
Q3 result to four decimal places) on 8,000 sampled MINDlarge-dev impressions:
hit rate 3.21\% (CI 2.83--3.61\%, consistent with the ceiling above), and among
the impressions where the retrieved 200 actually contain the real click, NRMS
re-ranking lifts AUC from 0.5428 (raw BM25 order, barely above random) to
\textbf{0.7224} --- paired $+0.1796$ [$+0.1375,+0.2213$], with MRR/nDCG@5/nDCG@10
all CI-clear wins in the same direction (\texttt{results/a2\_q2/README.md}). An
earlier interim check, run while this session's environment could not yet reach
the GPU cluster holding the trained checkpoint, used a deliberately-flagged
1-epoch/20,000-impression local checkpoint and found no significant effect
(paired AUC $-0.0464$, CI spanning zero) --- exactly the undertrained-checkpoint
artifact that check was labeled as, now superseded by the real result. The
honest overall answer: re-ranking helps substantially \emph{when} retrieval
succeeds, but cannot recover the $\sim$97\% of clicks retrieval never surfaces
at all --- both halves of Q2's answer, not one standing in for the other.
...
\textbf{Shipped as the new MIND leaderboard submission --- a real, confirmed win.}
The $-0.0060$ diversity regression is real (above) but is relative to the
reproduced control, not to what was actually deployed: measured against
Candidate J's real deployed diversity@10 (0.7840, from J's own submitted
predictions), the treatment is a diversity \emph{improvement} of $+0.0443$ ---
an order of magnitude larger than the regression it costs relative to the
lab-only control (ADR-015's 2026-09-14 addendum). Real MINDlarge-test
predictions were generated from the actual trained checkpoint (2,370,727
lines, independently verified: exact line-count match, every line a valid
permutation, zero duplicates or malformed rows) and packaged as
\texttt{prediction.zip}. A first upload (submission 930353) scored an
anomalous 0.5589 --- a real pipeline bug (a user-id key mismatch silently
scored every impression with empty history), root-caused via an independent
re-scoring spot check and fixed at the source (ADR-015's 2026-09-18
addendum), not a genuine generalization failure. The corrected file,
regenerated from the same unmodified checkpoint, was uploaded as submission
930992: \textbf{real Codabench score 0.6766} --- inside this project's normal
dev-to-test compression range (0.6868 $\to$ 0.6766) and a real $+0.0304$ over
the previous official submission (901961, 0.6462). 930992 is now the selected
official MIND submission.
