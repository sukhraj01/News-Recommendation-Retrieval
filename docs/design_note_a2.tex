...
A full-scale run (the actual reproduced/treatment NRMS re-ranking A1's
retrieval on MINDlarge-dev) was queued but blocked this session by an
environment access gap (the working environment could not reach the GPU
cluster holding the trained checkpoint --- see ADR-015). What is reported
instead is a real, honestly-labeled interim check with a reduced-scale local
checkpoint (1 epoch, 20{,}000 impressions, CPU): after-vs-before AUC
$-0.0464$ (CI $-0.1154$, $+0.0246$), not significant. This is evidence about
an undertrained checkpoint, not evidence that re-ranking fails --- the
decisive run needs the full-scale checkpoint and is specified, ready to run,
in \texttt{results/a2\_q2/README.md}.
