...
\date{\small \today\ (MIND treatment result pending --- see \S2.2)}
...
\subsection*{2.2 MIND --- control complete, treatment pending}
...
deliberately degraded control, not an improvement. Training is a $\sim$36--37h
job (measured: 300 samples/s at width 80 vs.\ 893 at width 30, a $2.98\times$
slowdown matching the $3.2\times$ predicted from a local smoke test), inside the
72h wall limit with $\sim$2$\times$ margin, expected to complete before this
deadline. \textbf{This section will be completed with the real paired result; it
is not filled in with a projection.}
...
\begin{tabular}{@{}lrrrr@{}}
\toprule
& Index/store memory & Request p99 & SLA 100ms & \$/1k queries \\
\midrule
MIND (M3 CPU) & 164.0\,MB & \textbf{94.67\,ms} & PASS, $1.1\times$ headroom & \$0.0031--\$0.0004 \\
EB-NeRD (\texttt{K\_rank\_nopos}) & 190.5\,MB & \textbf{22.74\,ms} & PASS, $4.4\times$ headroom & \$0.0009--\$0.0001 \\
\bottomrule
\end{tabular}\par}
\end{center}
...
\textbf{MIND's p99 headroom is thin, and its observed max (126\,ms) already
breaches the 100\,ms target.} NRMS is 78\% of a request's cost. EB-NeRD is
comfortable (4.4$\times$) and dominated by feature-frame construction, not the
booster.

Cost/QPS is reported as a \emph{range}, not a point: the optimistic end assumes
perfect linear scaling across an instance's vCPUs, which this project's own
profiling contradicts (a measured $2.5\times$ slowdown from a single concurrent
job on one 8GB machine, ADR-014) --- real throughput is sublinear, so the truth
sits between the two columns. Prices are AWS on-demand list, us-east-1, captured
2026-09-12; a GPU-measured MIND figure is an explicit, recorded gap (blocked
behind a 1-GPU cluster quota while the treatment trains), not estimated.

\section{Where the System Breaks at 10$\times$}

\textbf{MIND breaks on latency, not memory.} The index (164\,MB) scales with
articles/vocabulary, not users --- MINDlarge-test's 120,959 articles project to
$\sim$250\,MB, still trivial. But p99 headroom is only $1.1\times$, and NRMS is
78\% of a request. Two measured, ready fixes exist rather than needing new
engineering: this project's own NRMS candidate-vector cache ($7.02\times$ on
CPU, no accuracy cost, ADR-014) or GPU serving ($8.98\times$, same ADR).
...
\begin{itemize}
\item MIND's treatment result and its paired A/B (\S2.2) are the one section
  genuinely pending a running job, not an unstarted analysis --- the
  methodology, guardrails, and comparison target are identical to \S2.1's.
\item The GPU-measured MIND cost row (\S3) is blocked behind a 1-GPU cluster
  quota until the treatment job releases it; recorded as a gap, not estimated
  from the CPU figure.
