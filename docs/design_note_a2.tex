...
\textbf{Shipped as the new MIND leaderboard submission.} The $-0.0060$ diversity
regression is real (above) but is relative to the reproduced control, not to
what was actually deployed: measured against Candidate J's real deployed
diversity@10 (0.7840, from J's own submitted predictions), the treatment is a
diversity \emph{improvement} of $+0.0443$ --- an order of magnitude larger than
the regression it costs relative to the lab-only control (ADR-015's 2026-09-14
addendum). Real MINDlarge-test predictions were generated from the actual
trained checkpoint (2,370,727 lines, independently verified: exact line-count
match, every line a valid permutation, zero duplicates or malformed rows) and
packaged as \texttt{prediction.zip}; the Codabench upload itself is the
engineer's own manual step, as for every prior submission in this project.
